...
public class ElasticSearchManager {
    ...
    private static final String INDEX = "9023987";
    ...
    public static class RemoveBidTask extends AsyncTask<Bid,Void,Boolean> {
        ...
        protected Boolean doInBackground(Bid... params) {
            ...
            verifyConfig();
            Boolean success = false;
            Bid item_to_delete = params[0];
            try {
                DocumentResult execute = client.execute(new Delete.Builder(item_to_delete.getBidId()).index(INDEX).type(BID_TYPE).build());
                if(execute.isSucceeded()) {
